...
task :post do
  abort("rake aborted: '#{CONFIG['posts']}' directory not found.") unless Dir.exist?(CONFIG['posts'])
  def env_date
    ENV['date'] ? Time.parse(ENV['date']) : Time.now
  end
...
namespace :theme do
  ...
  task :install do
    ...
    unless Dir.exist?(packaged_theme_path)
      abort("rake aborted!
    ...
    Dir.chdir(packaged_theme_path) do
      Dir.glob('**/*.*') do |f|
        next if Dir.exist?(f) || f =~ /^(manifest|readme|packager)/i
  ...
  task :update do
    ...
    puts "=> #{name} theme has been updated!"
    puts '=> ---'
  end
  ...
  task :package do
    ...
    abort("rake aborted: '#{theme_path}' directory not found.") unless Dir.exist?(theme_path)
    abort("rake aborted: '#{asset_path}' directory not found.") unless Dir.exist?(asset_path)
...
def theme_git_pull(name)
  path = JB::Path.build(:theme_packages, node: name)
  Dir.chdir(path) do
